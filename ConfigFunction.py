...
def clip_or_pad_image(image, target_width,target_height):
    current_height,current_width = image.shape

    # 裁剪高度和宽度（如果需要）
    clipped_image = image[:min(current_height, target_height), :min(current_width, target_width)]

    # 计算需要填充的高度和宽度
    pad_height = max(0, target_height - clipped_image.shape[0])
    pad_width = max(0, target_width - clipped_image.shape[1])
...
def cloud_detection(Seg_path, Cloud_path, Cloud_Removal, Water_Occur_path, Water_Occur_removal):
    ...
    for file in files:
        if file.split('.')[-1] != 'tif':
            continue
        image = gdal.Open(os.path.join(Seg_path, file)).ReadAsArray()
        cutImg = clip_or_pad_image(image, height, width)

        water_img = gdal.Open(Water_Occur_path).ReadAsArray()
        water_img = clip_or_pad_image(water_img, height, width)
        ...
        cloud_resz = cv2.resize(cloud_img, (height, width))
        print("image size: ", cloud_resz.shape)
        ...
        Water_removal_dir = os.path.join(Water_Occur_removal,
                                         file.split('.')[0] + '.tif')  # file.split('_')[0][0:8]+ '.png' )
