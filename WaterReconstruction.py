...
def water_reconstruction_Globle_Rate(Cloud_Removal, Water_Occur_removal, Water_Occur_path, rate_path, Water_reconstruction, thr):
    ...
    for file in cfiles:
        if file.split('.')[-1] == 'tif' or file.split('.')[-1] == 'png':
            col.append(file.split('.')[0])
            col.append(file.split('.')[0] + "_occurrence")
            col.append(file.split('.')[0] + "_filter")
    datas = pd.read_csv(rate_path, usecols=col)
    for file in cfiles:
        if file.split('.')[-1] == 'tif' or file.split('.')[-1] == 'png':
            img_dir = os.path.join(Cloud_Removal, file)
            ...
            for i in range(0, len(datas[file.split('.')[0]])):
                if datas[file.split('.')[0]+ "_filter"][i] > thrRate:
                    thrWater = i
                    break
            print("unique value threshold: ", thrWater)
            cloud_pixels = set(map(tuple, np.column_stack(np.where((seg_img == 2) | (seg_img == -1)))))
            water_pixels = np.where((Water_Occur[:, :] > thrWater))
            water_pixels = set(map(tuple, np.column_stack(water_pixels)))
            intersection = cloud_pixels.intersection(water_pixels)
...
def water_Rate_filter(seg_img, water_img, rt):
    '''
    局部阈值计算
    '''
    rates = []
    ...
    for key in sorted_keys:
        ...
        if not key in dic.keys():
            ...
            continue
        rate_row.append(int(dic[key]) / int(dic1[key]))
    rates.append(occ_row)
    window_size = int(seg_img.shape[0]/20) # 30
    if len(rate_row) > 1:
        rate_r = pd.Series(rate_row)
        smoothed_rates = rate_r.rolling(window=window_size, min_periods=1, center=True).mean()
        # print(np.max(rate_r),np.max(smoothed_rates))
        # filter_row = np.convolve(rate_row, np.ones(window_size) / window_size, mode='same')
        rates.append(smoothed_rates)
    else:
        rates.append(rate_row)
    for i in range(len(rates[1])):
        if rates[1][i] > rt:  #0.2: #
            # print(len(rates[0]),len(rates[1]),i)
            return rates[0][i]
    return -1


def water_reconstruction_Local_filter(Cloud_Removal, Water_Occur_removal, Water_Occur_path, rate_path,
                                      Water_reconstruction, rt):
    ...
    Water_Occur = gdal.Open(Water_Occur_path).ReadAsArray().astype(float)
    Water_Occur = cv2.resize(Water_Occur, (height, width))
    maxwo = np.max(Water_Occur)
    NoWaterPixs = np.where(Water_Occur == 0)
    ...
    for file in cfiles:
        if file.split('.')[-1] == 'tif' or file.split('.')[-1] == 'png':
            rcns_dir = os.path.join(Water_reconstruction, file.split('.')[0] + '.' + file.split('.')[1] + '.tif')
            print(file, "is being reconstructed...")
            if os.path.exists(rcns_dir):
                continue
            img_dir = os.path.join(Cloud_Removal, file)
            seg_img = gdal.Open(img_dir).ReadAsArray()
            ...
            for i in range(0, len(datas[file.split('.')[0]])):
                if datas[file.split('.')[0] + "_filter"][i] > thrRate:
                    thrWater = datas[file.split('.')[0] + "_occurrence"][i]
                    break
            ...
            # count150 = 0
            count = 0
            tenth = len(g_array) // 50
            for pnt in g_array:
                if count % tenth == 0:
                    print(count/len(g_array))
                    cv2.imwrite(rcns_dir, seg_img)
                ptx = pnt[0]
                pty = pnt[1]
                wo = Water_Occur[ptx, pty]
                if wo == 0:
                    ...
                    if len(valid1_50[0]) > 1250:
                        water50 = water_img_pad150[pnt[0] + 50:pnt[0] + 100, pnt[1] + 50:pnt[1] + 100]
                        thrsh_pix = water_Rate_filter(seg50, water50, rt)
                        # count50 += 1
                    else:
                        seg100 = seg_img_pad150[pnt[0] + 25:pnt[0] + 125, pnt[1] + 25:pnt[1] + 125]
                        valid1_100 = np.where((seg100 == 1) | (seg100 == 0))
                        if len(valid1_100[0]) > 5000:
                            water100 = water_img_pad150[pnt[0] + 25:pnt[0] + 125, pnt[1] + 25:pnt[1] + 125]
                            thrsh_pix = water_Rate_filter(seg100, water100, rt)
                            # count100 += 1
                        else:
                            seg150 = seg_img_pad150[pnt[0]:pnt[0] + 150, pnt[1]:pnt[1] + 150]
                            valid1_150 = np.where((seg150 == 1) | (seg150 == 0))
                            if len(valid1_150[0]) > 11250:
                                water150 = water_img_pad150[pnt[0]:pnt[0] + 150, pnt[1]:pnt[1] + 150]
                                thrsh_pix = water_Rate_filter(seg150, water150, rt)
                                # count150 += 1
                            else:
                                thrsh_pix = thrWater
